...
// All acknowledgement request-handling logic lives here rather than in
// route.ts directly. Next.js's route type-checker only permits a fixed
// set of exports (HTTP method handlers, dynamic/revalidate config) from
// a route.ts file -- an extra exported helper like
// handleAcknowledgeRequest fails `next build`'s route validation. Moving
// it here lets route.ts stay a thin, Next-compliant wrapper while this
// module (and its exported handleAcknowledgeRequest) stays directly
// importable and testable by route.test.ts against a fake Supabase
// client, without a live database or module mocking.
...
async function acknowledgeCutting(supabase: SupabaseServerClient, recordId: string, acknowledgedAt: string) {
    ...
    return NextResponse.json(
      {
        ...
        record: normalizeCuttingForCommerce(cuttingRow as CuttingCommerceSource),
  ...
  if (existingCutting) {
    return describeAcknowledgeFailure("cutting", existingCutting as CuttingCommerceSource, normalizeCuttingForCommerce);
  }
...
async function acknowledgeMother(supabase: SupabaseServerClient, recordId: string, acknowledgedAt: string) {
  ...
  if (existingMother) {
    const facts = await lookupMotherFacts(supabase, recordId);
    return describeAcknowledgeFailure("mother", { ...existingMother, archived_at: null } as MotherCommerceSource, (row) =>
...
export async function handleAcknowledgeRequest(
  request: NextRequest,
  recordId: string,
  supabase: SupabaseServerClient
) {
...
function describeAcknowledgeFailure<T extends { commerce_selected_at: string | null; commerce_acknowledged_at: string | null }>(
  ...
  if (existing.commerce_acknowledged_at) {
    return NextResponse.json(
      { plantRecordType, record: normalize(existing), alreadyAcknowledged: true },
      { headers: { "Cache-Control": "no-store" } }
    );
  }
