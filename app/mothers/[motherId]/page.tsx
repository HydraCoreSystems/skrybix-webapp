...
export default async function MotherProfilePage({ params }: { params: { motherId: string } }) {
  ...
  return <div className="mother-profile">
    <div className="profile-breadcrumbs"><Link href="/mothers">← Mother Plants</Link><span>{mother.mother_id}</span></div>
    <section className="card mother-profile-hero"><div><p className="eyebrow">Mother plant profile</p><h1>{mother.display_name}</h1><code>{mother.mother_id}</code><p className="profile-botanical">{[mother.botanical_line1, mother.botanical_line2].filter(Boolean).join(" · ") || "Botanical label not recorded"}</p></div><div className="profile-primary-actions"><Link className="btn" href={`/cuttings/new?mother=${encodeURIComponent(mother.mother_id)}`}>Take cuttings</Link><Link className="btn secondary" href={`/mothers/${encodeURIComponent(mother.mother_id)}/edit`}>Edit profile</Link></div></section>
