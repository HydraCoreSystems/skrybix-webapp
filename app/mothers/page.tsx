  ...
  return <div className="card">
    ...
    {rows.length === 0 ? <div className="worklist-empty"><strong>No mother plants match this work queue.</strong><p>Clear the filters or choose another queue.</p></div> : <div className="mother-worklist">{rows.map((mother) => {
      ...
      return <article className="mother-worklist-card" key={mother.mother_id}>
        <div className="mother-identity"><div><Link href={`/mothers/${encodeURIComponent(mother.mother_id)}`}><strong>{mother.display_name}</strong></Link><code>{mother.mother_id}</code></div><span className={`status-pill ${mother.sold ? "queued" : "commerce-acknowledged"}`}>{mother.sold ? "Marked sold" : "In collection"}</span></div>
