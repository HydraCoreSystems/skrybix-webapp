...
-- changed, only inserted. This is deliberately NOT relying on "no
-- application code happens to update it" -- the DB itself refuses.
create or replace function forbid_commerce_sku_update()
returns trigger language plpgsql as $$
begin
  raise exception 'commerce_skus rows are immutable and cannot be updated (id=%)', old.id;
end;
$$;
...
create trigger commerce_skus_no_update
  ...
  for each row execute function forbid_commerce_sku_update();
...
create or replace function assign_commerce_sku_for_cutting(
  p_source_record_id text,
  p_mother_source_record_id text,
  p_genus_code char(2),
  p_plant_code text
)
returns text language plpgsql as $$
...
begin
  ...
  v_mother_sku := assign_commerce_sku_for_mother(p_mother_source_record_id, p_genus_code, p_plant_code);
  ...
  where m.plant_record_type = 'mother' and m.source_record_id = p_mother_source_record_id
  on conflict (plant_record_type, source_record_id) do nothing;
...
declare
  v_sku text;
begin
  v_sku := assign_commerce_sku_for_cutting(p_cutting_id, p_mother_id, p_genus_code, p_plant_code);
